...
const CheckNumbers = () => {
    ...
    const runEngine = (submittedNum) => {
        if(submittedNum&&submittedNum.full){
            ...
            firstTwoSumAndLastTwoSum(submittedNum, picks.slice(0, 20)).then(x=>x>=1&&similaritiesFunc('similar firstTwoAndLastTwoSum'))
            ...
            previousCombFirstTwo(submittedNum, picks.slice(0, 9)).then(x=>x>=1&&similaritiesFunc('similar previousFirstTwo'))

            //ex submitted number   = 3, 2, 5
            //newest winning number = 4, 1, 6
            //[2+1 = 3 ] [5+6 = 11 ] | last two numbers are '311'
            previousCombLastTwo(submittedNum, picks.slice(0, 9)).then(x=>x>=1&&similaritiesFunc('similar previousLastTwo'))

            //ex submitted number   = 3, 2, 5
            //newest winning number = 4, 1, 6
            //[3+4 = 7 ] [2+1 = 3 ] [5+6 = 11 ] | 7+3+11 = sumFinal is 21
            previousCombSumFinal(submittedNum, picks.slice(0, 9)).then(x=>x>=1&&similaritiesFunc('similar previousSumFinal'))

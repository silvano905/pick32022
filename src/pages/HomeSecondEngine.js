...
function SecondEngine() {
    ...
    const runEngine = async () => {
        ...
        while (runWhile&&finalList.length<9) {
            ...
            let four = await firstLastSimilar(randomCombination.full, picks.slice(0, 10))
            if(four>=1){
            ...
            let five = await firstTwoSumAndLastTwoSum(randomCombination, picks.slice(0, 10))
            if(five>=1){
            ...
            if(randomCombination&&randomCombination.full) {
                ...
                let eight = await previousCombFirstTwo(randomCombination, picks.slice(0, 5))
                if(eight>=1){
                ...
                let nine = await previousCombLastTwo(randomCombination, picks.slice(0, 5))
                if(nine>=1){
                ...
                let ten = await previousCombSumFinal(randomCombination, picks.slice(0, 5))
                if(ten>=1){
                ...
                // let eleven = await previousCombOneTwoThree(randomCombination, picks.slice(0, 10))
                // if(eleven>=1){
